...
foreach ($entry in $sortedResults) {
    Write-Host ("{0,-43} {1,15}" -f $entry.Subscription, "$($entry.UsageUSD) USD") -ForegroundColor Cyan
    ...
    if ($entry.TopServices -ne "No data" -and $entry.TopServices -ne "Error") {
        $entry.TopServices -split ', ' | ForEach-Object {
            ...
            Write-Host ("   - {0,-38} {1,15}" -f $serviceName, $serviceUsageFormatted) -ForegroundColor Gray
        }
    } else {
